...
import { getPendingOfferList } from '../../../redux/actions';
//import axios from 'axios';
...
const ViewOfferDataList = (props) => {
    let loginDetails = useSelector((state)=> state.Auth.user || []);
    let records = useSelector((state)=> state.PendingApproval.pendingoffer || []);
    var getUsername = loginDetails.Username;
    const dispatch = useDispatch(); 
    //let records =  [];
  ////console.log(records, 'join list');
  useEffect(() => {
    dispatch(getPendingOfferList(getUsername));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/ViewPendingOfferList', path: '/ViewPendingOfferList', active: true },
                      ]}
                    />
                </Col>
            ...
            {/* <Col className="text-right" lg={12}>
                 <button className="btn btn-secondary bg-secondary upbtn  mr-0">Refresh</button>
...
export default ViewOfferDataList;
