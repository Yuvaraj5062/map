...
import {getYesterdayInterviewList } from '../../../redux/dashboardTables/yesterday/actions';
...
const YesterdayInterview = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.yesterdayList.yesterdayinterview || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getYesterdayInterviewList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/YesterdayInterview', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
...
export default YesterdayInterview;
