...
import { getYesterdaySubmissionList } from '../../../redux/dashboardTables/yesterday/actions';
...
const YesterdaySubmission = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.yesterdayList.yesterdaysubmission || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
   // //console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getYesterdaySubmissionList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/YesterdaySubmission', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
...
export default YesterdaySubmission;
