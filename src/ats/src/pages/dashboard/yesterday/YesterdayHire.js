...
import { getYesterdayHireList } from '../../../redux/dashboardTables/yesterday/actions';
...
const YesterdayHire = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.yesterdayList.yesterdayhire || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getYesterdayHireList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/YesterdayHire', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
