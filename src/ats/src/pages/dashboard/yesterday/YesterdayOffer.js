...
import { getYesterdayOfferList } from '../../../redux/dashboardTables/yesterday/actions';
...
const YesterdayOffer = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.yesterdayList.yesterdayoffer || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getYesterdayOfferList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/YesterdayOffer', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
