...
import { getMonthSubmissionList } from '../../../redux/dashboardTables/month/actions';
...
const MonthSubmission = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.monthList.monthsubmission || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
   // //console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getMonthSubmissionList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/MonthSubmission', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
...
export default MonthSubmission;
