...
import { getMonthOfferList } from '../../../redux/dashboardTables/month/actions';
...
const MonthOffer = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.monthList.monthoffer || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getMonthOfferList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/MonthOffer', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
