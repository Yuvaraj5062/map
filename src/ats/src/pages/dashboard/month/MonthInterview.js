...
import {getMonthInterviewList } from '../../../redux/dashboardTables/month/actions';
...
const MonthInterview = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.monthList.monthinterview || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getMonthInterviewList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/MonthInterview', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
