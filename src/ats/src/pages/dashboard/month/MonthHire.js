...
import { getMonthHireList } from '../../../redux/dashboardTables/month/actions';
...
const MonthHire = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.monthList.monthhire || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getMonthHireList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/MonthHire', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
