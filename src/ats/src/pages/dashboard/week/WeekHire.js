...
import { getWeekHireList } from '../../../redux/dashboardTables/week/actions';
...
const WeekHire = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.weekList.weekhire || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getWeekHireList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/WeekHire', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
