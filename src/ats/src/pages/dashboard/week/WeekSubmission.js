...
import { getWeekSubmissionList } from '../../../redux/dashboardTables/week/actions';
...
const WeekSubmission = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.weekList.weeksubmission || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
   // //console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getWeekSubmissionList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/WeekSubmission', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
