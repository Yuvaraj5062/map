...
import { getWeekOfferList } from '../../../redux/dashboardTables/week/actions';
...
const WeekOffer = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.weekList.weekoffer || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getWeekOfferList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/WeekOffer', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
