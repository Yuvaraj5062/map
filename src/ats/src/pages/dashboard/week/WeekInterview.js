...
import {getWeekInterviewList } from '../../../redux/dashboardTables/week/actions';
...
const WeekInterview = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.weekList.weekinterview || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getWeekInterviewList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/WeekInterview', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
