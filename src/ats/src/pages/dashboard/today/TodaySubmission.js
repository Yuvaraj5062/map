...
import { getTodaySubmissionList } from '../../../redux/dashboardTables/today/actions';
...
const TodaySubmission = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.todayList.todaysubmission || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
   // //console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getTodaySubmissionList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/TodaySubmission', path:'/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
