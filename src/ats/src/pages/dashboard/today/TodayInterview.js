...
import {getTodayInterviewList } from '../../../redux/dashboardTables/today/actions';
...
const TodayInterview = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.todayList.todayinterview || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getTodayInterviewList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/TodayInterview', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
