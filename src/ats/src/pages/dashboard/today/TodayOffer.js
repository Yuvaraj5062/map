...
import { getTodayOfferList } from '../../../redux/dashboardTables/today/actions';
...
const TodayOffer = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.todayList.todayoffer || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getTodayOfferList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/TodayOffer', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
