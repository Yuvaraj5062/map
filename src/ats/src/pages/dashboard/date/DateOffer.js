...
import { getDateOfferList } from '../../../redux/dashboardTables/date/actions';
...
const DateOffer = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.dateList.dateoffer || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
       // alert('call')
        dispatch(getDateOfferList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/TillDateOffer', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
