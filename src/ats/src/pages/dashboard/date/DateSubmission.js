...
import { getDateSubmissionList } from '../../../redux/dashboardTables/date/actions';
...
const DateSubmission = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.dateList.datesubmission || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
   // //console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getDateSubmissionList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/TillDateSubmission', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
