...
import {getDateInterviewList } from '../../../redux/dashboardTables/date/actions';
...
const DateInterview = (props) => {
    const dispatch = useDispatch();
    let records = useSelector((state) => state.dateList.dateinterview || []);
    let userDeatails = useSelector((state) => state.Auth.user || []);
    ////console.log(records, 'requirement');
    useEffect(() => {
        dispatch(getDateInterviewList(userDeatails.Username));
        ...
        <React.Fragment>
            <Row className="page-title">
                <Col md={6}>
                    <PageTitle
                      breadCrumbItems={[
                          { label: 'Dashboard/TillDateInterview', path: '/', active: false },
                      ]}
                    />
                </Col>
                <Col md={6} className="text-right">
