...
export const DownloadSalaryRenya = () => {
    ...
    const handleSubmit = e => {
        setLoader(true)
        e.preventDefault()
        axios.get(urlpattern + `GenerateReynaSalaryExcelSheet?salarymonth=${month}&salaryyear=${year}`).then((res) => {
            if (res.status === 200) {
                setLoader(false)
                window.open(urlpattern + `GenerateReynaSalaryExcelSheet?salarymonth=${month}&salaryyear=${year}`)
